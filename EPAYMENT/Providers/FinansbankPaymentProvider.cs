...
namespace EPAYMENT.Providers
{
    public class FinansbankPaymentProvider : IPaymentProvider
    {
        public PaymentParameterResult GetPaymentParameters(PaymentRequest request)
        {
            ...
            string mbrId = "5";//Mağaza numarası
            string merchantId = request.ClientId;//Mağaza numarası
            string userCode = request.Username;//
            string userPass = request.Password;//Mağaza anahtarı
            ...
            try
            {
                ...
                parameterResult.PaymentUrl = new Uri("https://vpos.qnbfinansbank.com/Gateway/3DHost.aspx");
            }
            catch (Exception ex)
            {
